...
routing.post('/register', (req, res, next) => {
  const {
    userId, password, name, city, email, phone,mname
  } = req.body;
  if (validator.validUserId(userId, next) && validator.validPassword(password, next)
    ...
    && validator.validEmail(email, next) && validator.validPhoneNo(phone, next) &&  validator.validMname(mname, next)) {
    const newUser = new NewUser(userId, password, name, city, email, phone,mname);
...
routing.post('/forgotPassword', (req, res, next) => {
  const { userId, mname } = req.body;
  if (validator.validateUserId(userId, next) && validator.validMname(mname, next)) {
    user.forgotLoginPassword(userId, mname, res, next);
  }
});
