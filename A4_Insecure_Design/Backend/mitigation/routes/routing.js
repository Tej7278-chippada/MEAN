...
routing.post('/getOTP', (req, res, next) => {
  const { userId } = req.body;
  if (validator.validateUserId(userId, next)) {
    user.getOTP(userId, res, next);
  }
});

routing.post('/verifyOTP', (req, res, next) => {
  const { userId ,otp} = req.body;
  if (validator.validateUserId(userId, next)) {
    user.verifyOTP(userId,otp, res, next);
  }
});
