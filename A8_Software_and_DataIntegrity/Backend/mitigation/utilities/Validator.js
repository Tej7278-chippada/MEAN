...
exports.validatePassword = (password, next) => {
  if (password.length === 0) {
    ...
    errorObject.message = 'Password has invalid characters';
    next(errorObject);
  } else {
    return true;
  }
...
exports.validateHash = (hash, data, next) => {
  const value = crypto.createHmac('sha256', 'password')
    .update(data)
    .digest('hex');
  if (value === hash) {
    return true;
  ...
  errorObject.message = 'Please resubmit the data';
  next(errorObject);
};
